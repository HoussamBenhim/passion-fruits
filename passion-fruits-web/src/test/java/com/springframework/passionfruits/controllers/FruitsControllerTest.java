...
class FruitsControllerTest {
	...
	void testGetAllProduct() throws Exception {
		...
		mockMvc.perform(get("/fruits/list")).andExpect(status().isOk()).andExpect(jsonPath("$[0].name", is("banane")));
	...
	void testfindFruitById() throws Exception {
		...
		mockMvc.perform(get("/fruits/1")).andExpect(status().isOk()).andExpect(jsonPath("$.name", is("banane")));
	}
